...
import cn.tanzhou.classmanagement.facade.api.combination.JurisCombinationApi;
...
public class AppTestDelete {
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 引用远程服务
        ReferenceConfig<JurisCombinationApi> reference = new ReferenceConfig<JurisCombinationApi>(); // 此实例很重，封装了与注册中心的连接以及与提供者的连接，请自行缓存，否则可能造成内存和连接泄漏
        reference.setApplication(application);
        reference.setRegistries(Arrays.asList(registry1,registry2,registry3));
        //reference.setRegistry(registry1);
        reference.setInterface(JurisCombinationApi.class);
        reference.setVersion("1.0.0");

        // 和本地bean一样使用xxxService
        JurisCombinationApi permissionFacadeAPI = reference.get(); // 注意：此代理对象内部封装了所有通讯细节，对象较重，请缓存复用
